...
function MainHistory(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const query = "?" + "idUser" + "=" + localStorage.getItem("id_user");
      ...
      <div className="table-responsive pt-5 pb-5">
        <table className="table">
          ...
          <tbody>
            {listCart &&
              listCart.map((value) => (
                <tr className="text-center" key={value.idOrder}>
                  <td className="align-middle border-0">
                    <p className="mb-0 small">{value.idOrder}</p>
                  </td>
                  <td className="align-middle border-0">
                  ...
                  <td className="align-middle border-0">
                    <p className="mb-0 small">
                      {!value.status ? "Waiting for pay" : value.status}
                    </p>
                  </td>
                  <td className="align-middle border-0">
                    <Link
                      className="btn btn-outline-dark btn-sm"
                      to={`/history/${value.idOrder}`}
                    >
                      View
